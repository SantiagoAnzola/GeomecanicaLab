...
namespace DanielLochner.Assets.SimpleSideMenu
{
    ...
    public class SimpleSideMenu : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler, IInitializePotentialDragHandler
    {
        ...
        private void Start()
        {
            content=GameObject.Find("ContentBottomInfo");
            Debug.Log("CONTENT Y---------> "+content.transform.position.y);
            if (IsValidConfig)
            {
                Setup();
            }
        ...
        public void OnInitializePotentialDrag(PointerEventData eventData)
        {
            ...
            isPotentialDrag = (isHandleDraggable && eventData.pointerEnter == handle) ||
                      (isMenuDraggable && eventData.pointerEnter == gameObject) ||
                      (content.transform.position.y < 3.6f);
